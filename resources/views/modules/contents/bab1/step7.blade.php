<section id="step-7" class="step-slide">
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-10 -mt-8">
        ...
        <p class="text-slate-600 mb-8 leading-relaxed">
            Selamat! Anda telah menyelesaikan materi 1. Sejenak, mari kita evaluasi pemahaman yang telah
            Anda dapatkan.
            Jawaban jujur Anda akan sangat membantu dalam proses belajar selanjutnya.
        </p>
        ...
        <div class="flex justify-between gap-4 mt-12">
            ...
            <button type="button" onclick="finishChapter()" class="btn-primary">Selesai Materi 1</button>
        </div>
    ...
    <script>
        function finishChapter() {
            ...
            if (!q1 || q1.trim() === "") {
                // Ganti alert biasa dengan SweetAlert Warning
                Swal.fire({
                    icon: 'warning',
                    title: 'Belum Lengkap',
            ...
            fetch("{{ route('submit.reflection') }}", {
                ...
                .then(data => {
                    ...
                    if (data.redirect) {
                        // --- INI BAGIAN POPUP KERENNYA ---
                        Swal.fire({
                            title: 'Luar Biasa!',
                            text: 'Bab 1 Selesai. Refleksi Anda telah tersimpan.',
                            icon: 'success',
                            ...
                            if (result.isConfirmed) {
                                window.location.href = data.redirect; // Redirect setelah klik OK
                            }
                        });
                    } else {
                        // Error dari server
