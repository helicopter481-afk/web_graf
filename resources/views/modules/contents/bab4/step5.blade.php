<section id="step-5" class="step-slide">
    ...
    <div id="quiz-wrapper">
        ...
        <div class="h-16 bg-white border-b border-slate-200 flex justify-between items-center px-6 shrink-0 z-30 shadow-sm">
            <div>
                <h2 class="font-bold text-slate-800 text-lg">Quiz Materi 4</h2>
                <p class="text-xs text-slate-500">Algoritma Dijkstra</p>
            </div>
        ...
        <div class="flex flex-1 overflow-hidden relative">
            ...
            <div id="area-soal" class="custom-scroll">
                <div class="max-w-6xl mx-auto w-full pt-6 pb-24">
                                ...
                                <div class="pl-1">
                                        ...
                                        @if (isset($soal->options) && is_iterable($soal->options))
                                            @foreach ($soal->options as $key => $val)
                                                ...
                                                <button type="button" class="choice-btn group" data-key="{{ $key }}"
                                                    onclick="q4_aksiPilihJawaban('{{ $qid }}', '{{ $key }}', this)">
                                            ...
                                            @foreach ($soal->options as $key => $val)
                                                <label class="choice-btn flex items-center gap-4 cursor-pointer">
                                                    <input type="checkbox" name="{{ $qid }}[]" value="{{ $key }}"
                                                        ...
                                                        onchange="q4_aksiCekStatus({{ $nomor }})">
                                                    <span class="text-sm font-medium">{{ $val }}</span>
                                                </label>
                                            @endforeach
                                        @endif
                                    ...
                                    @elseif($soal->type == 'NUMBER' || $soal->type == 'FILL_BLANK')
                                        <input type="text" id="{{ $qid }}-input"
                                            ...
                                            placeholder="Ketik jawaban..." oninput="q4_aksiCekStatus({{ $nomor }})" autocomplete="off" spellcheck="false">
                                            ...
                                            @for ($i = 0; $i < $jml; $i++)
                                                <div class="flex gap-2 mb-2 items-center">
                                                    ...
                                                    <input type="text"
                                                        ...
                                                        oninput="q4_aksiCekStatus({{ $nomor }})" placeholder="..." spellcheck="false">
                                                </div>
                                            @endfor
                                        </div>
                    ...
                    @else
                        <div class="text-center py-20 text-slate-500">
                            <p class="font-medium">Data soal Quiz Materi 4 belum tersedia di database.</p>
                            <p class="text-xs mt-2">Silakan tambahkan soal dengan section code <strong>quiz4</strong> di halaman Admin.</p>
                        </div>
                    @endif
                ...
                <div id="navigasi-bawah" class="fixed bottom-0 left-0 right-0 md:right-[320px] h-20 bg-white border-t border-slate-200 px-8 flex justify-between items-center z-40">
                    <button id="btn-prev" onclick="q4_aksiGantiSoal(-1)" class="px-5 py-2.5 border border-slate-300 rounded-lg text-slate-600 font-bold text-sm hover:bg-slate-50 disabled:opacity-50">SEBELUMNYA</button>
                    <button id="btn-next" onclick="q4_aksiGantiSoal(1)" class="px-6 py-2.5 bg-slate-900 text-white rounded-lg font-bold text-sm hover:bg-slate-800 shadow-lg">SELANJUTNYA</button>
                </div>
            </div>
            ...
            <div id="sidebar-nav" class="hidden md:flex flex-col h-full bg-white border-l border-slate-200 w-[320px] shrink-0 z-30">
                ...
                <div class="p-5 flex-1 overflow-y-auto custom-scroll">
                    <div class="grid grid-cols-4 gap-3">
                        @if (isset($quiz4))
                            @foreach ($quiz4 as $index => $soal)
                                <button onclick="q4_aksiBukaSoal({{ $index + 1 }})" id="nav-btn-{{ $index + 1 }}" class="nav-btn h-10 rounded-lg font-bold text-sm transition flex items-center justify-center">
                                    {{ $index + 1 }}
                                </button>
                            @endforeach
                        @endif
                    ...
                    <div class="mt-6 space-y-3">
                        <button onclick="q4_toggleRagu(window.q4_soalAktif)" class="w-full py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 text-sm font-bold rounded-lg transition">Tandai Ragu-Ragu</button>
                        <button onclick="q4_aksiCekSelesai()" class="w-full py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-bold rounded-lg transition shadow-md">KUMPULKAN</button>
                    </div>
                    {{-- Legend --}}
        ...
        <div id="modal-mulai" class="fixed inset-0 min-h-screen bg-slate-900/80 z-[9999] flex justify-center p-4 backdrop-blur-sm items-center w-full ">
            <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] flex flex-col">
                ...
                <div class="bg-slate-900 text-white p-4 md:p-5 text-center border-b-4 border-blue-500 shrink-0 rounded-t-2xl">
                    <h2 class="text-xl md:text-2xl font-black tracking-wider uppercase">QUIZ MATERI 4 : DIJKSTRA</h2>
                    <p class="text-slate-300 text-xs md:text-sm mt-1">Uji Pemahaman Teori & Logika Relaxation</p>
                </div>
                    ...
                    <div class="flex items-start gap-3 p-3 md:p-4 bg-blue-50 border border-blue-200 rounded-xl shadow-sm">
                        ...
                        <div>
                            ...
                            <p class="text-xs md:text-sm text-blue-800 leading-relaxed text-justify">
                                Terdapat <strong>{{ isset($quiz4) ? count($quiz4) : 0 }} Tantangan</strong> yang harus Anda selesaikan. Fokus pada kemampuan Anda membaca dan menerjemahkan logika graf ke dalam bahasa pemrograman Python.
                            </p>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <ul class="space-y-3 text-sm text-justify text-slate-700">
                            ...
                            <li class="flex items-start gap-3">
                                ...
                                <span class="leading-relaxed pt-0.5"><strong>Format Code (Penting!):</strong> Pada soal isian teks, perhatikan dengan sangat teliti penggunaan tanda kurung siku <code>[]</code>, kurung biasa <code>()</code>, dan tanda kutip <code>" "</code>.</span>
                            </li>
                            <li class="flex items-start gap-3">
                                ...
                                <span class="leading-relaxed pt-0.5"><strong>Navigasi:</strong> Gunakan panel di sebelah kanan layar untuk berpindah soal. Tombol <strong>Tandai Ragu-ragu</strong> berguna jika Anda ingin mengecek ulang.</span>
                            </li>
                            <li class="flex items-start gap-3">
                                <span class="w-6 h-6 rounded-full bg-red-100 border border-red-200 flex items-center justify-center text-red-600 font-bold shrink-0 text-xs"><i class="fa-solid fa-ban">4</i></span>
                                <span class="leading-relaxed pt-0.5 text-red-700"><strong>Dilarang Keras:</strong> Jangan merefresh halaman browser (F5) saat kuis berlangsung agar progres Anda tidak terhapus.</span>
                            </li>
                        </ul>
                    </div>
                </div>
                ...
                <div class="bg-slate-50 p-4 md:p-5 border-t border-slate-200 flex justify-center shrink-0 rounded-b-2xl">
                    <button onclick="q4_aksiMulaiUjian()" class="w-full md:w-auto px-8 py-3 bg-blue-600 hover:bg-blue-600 text-white font-bold rounded-xl shadow-lg transition-transform active:scale-95 text-sm md:text-base flex justify-center items-center gap-3">
...
<script>
    ...
    @php
        ...
        $isDone = false;
        $rawAnswers = null;
        $quizData = !empty($quiz4) ? $quiz4 : []; // Pastikan ini tidak null

        if (isset($progress) && isset($progress['quiz4'])) {
            $dataQuiz = $progress['quiz4'];
    ...
    // --- VARIABEL GLOBAL ---
    window.q4_hasHistory = {{ $isDone ? 'true' : 'false' }};
    window.q4_historyScore = {{ $displayScore }};
    window.q4_dataSoal = @json($quizData);

    // VARIABEL STATE
    window.q4_totalSoal = window.q4_dataSoal ? window.q4_dataSoal.length : 0;
    window.q4_isExamMode = false;
    window.q4_soalAktif = 1;
    window.q4_sisaDetik = 600;
    window.q4_timerUjian = null;
    window.q4_dbRagu = {};

    // PARSING JAWABAN DENGAN SAFEKEEPING
    window.q4_dbJawabanUser = {};
    ...
    try {
        ...
        if (rawData) {
            ...
            window.q4_dbJawabanUser = tempDb;
        }
    } catch (e) {
    ...
    document.addEventListener("DOMContentLoaded", function() {
        if (window.q4_hasHistory) {
            ...
            if (elSkor) elSkor.innerText = window.q4_historyScore;
            ...
            setTimeout(q4_restoreUserAnswers, 500);
        } else {
            if (window.q4_totalSoal > 0) {
                q4_aksiBukaSoal(1);
                if (typeof q4_initDragAndDrop === "function") q4_initDragAndDrop();
            }
        }
    });
    ...
    // ==========================================
    function q4_restoreUserAnswers() {
        if (!window.q4_dbJawabanUser || Object.keys(window.q4_dbJawabanUser).length === 0) return;

        Object.keys(window.q4_dbJawabanUser).forEach(key => {
            let answer = window.q4_dbJawabanUser[key];
        ...
        q4_updateTampilanNav();
    }

    // ==========================================
    // 4. FUNGSI REVIEW (STRICT ANTI NYONTEK)
    // ==========================================
    function q4_aksiReviewJawaban() {
        ...
        document.querySelectorAll('[onclick^="q4_toggleRagu"], [onclick="q4_aksiCekSelesai()"]').forEach(btn => btn.style.display = "none");
        document.getElementById('sidebar-nav').classList.add('pointer-events-auto');

        q4_restoreUserAnswers();

        // GRADING & VISUALISASI
        window.q4_dataSoal.forEach((soal, index) => {
            let nomor = index + 1;
            let qid = 'soal_' + nomor;
            let container = document.getElementById('container-' + nomor);
            ...
            let userAnswer = window.q4_dbJawabanUser[qid];
            ...
            // KOTAK PEMBAHASAN TANPA BOCORAN JIKA SALAH + FIX CHECKBOX
            if (feedbackArea) {
    ...
    // 5. HELPER & HANDLERS
    // ==========================================
    function q4_updateTampilanNav() {
        for (let i = 1; i <= window.q4_totalSoal; i++) {
            ...
            let terisi = q4_cekApakahTerisi(i);
    ...
    function q4_cekApakahTerisi(nomor) {
        let qid = 'soal_' + nomor;
        let data = window.q4_dbJawabanUser[qid];
    ...
    function q4_aksiBukaSoal(nomor) {
        ...
        if (next) {
            if (nomor === window.q4_totalSoal) next.classList.add('hidden');
    ...
    function q4_aksiGantiSoal(arah) {
        let next = window.q4_soalAktif + arah;
        if (next >= 1 && next <= window.q4_totalSoal) q4_aksiBukaSoal(next);
    }

    function q4_toggleRagu(nomor) {
        window.q4_dbRagu['soal_' + nomor] = !window.q4_dbRagu['soal_' + nomor];
        q4_updateTampilanNav();
    }

    function q4_aksiPilihJawaban(qid, nilai, btn) {
        if (!window.q4_isExamMode) return;
        window.q4_dbJawabanUser[qid] = nilai;
    ...
    function q4_aksiCekStatus(nomor) {
        let qid = 'soal_' + nomor;
        let container = document.getElementById('container-' + nomor);
        ...
        if (checkboxes.length > 0) {
            ...
            if (vals.length > 0) window.q4_dbJawabanUser[qid] = vals;
            else delete window.q4_dbJawabanUser[qid];
        } else {
            ...
            if (texts.length > 0) {
                ...
                if (adaIsi) window.q4_dbJawabanUser[qid] = (texts.length === 1) ? vals[0] : vals;
                else delete window.q4_dbJawabanUser[qid];
            }
        }
        q4_updateTampilanNav();
        ...
        zones.forEach(zone => {
            ...
            zone.addEventListener('drop', (e) => {
                ...
                if (containerSoal) {
                    ...
                    if (adaIsi) window.q4_dbJawabanUser[qid] = answerObj;
                    else delete window.q4_dbJawabanUser[qid];
                    q4_updateTampilanNav();
                }
            });
    ...
    function q4_aksiCekSelesai() {
        let kosong = [];
        for (let i = 1; i <= window.q4_totalSoal; i++) {
            if (!q4_cekApakahTerisi(i)) kosong.push(i);
        }
    ...
    function q4_aksiFinalisasi() {
        ...
        fetch("{{ route('submit.quiz') }}", {
                ...
                body: JSON.stringify({
                    ...
                    section_code: 'quiz4',
                    score: 0, 
                    answers: window.q4_dbJawabanUser 
                })
            })
            .then(res => res.json())
            .then(data => {
                ...
                window.q4_hasHistory = true;
                window.q4_historyScore = data.score;
            })
            .catch(err => {
                console.error(err);
    ...
    function q4_aksiUlangi() {
        ...
        function restoreModalUI() {
            ...
            if(btnUlangi) {
                ...
                btnUlangi.onclick = q4_aksiUlangi;
            }
            if(btnLihat) {
                btnLihat.innerText = "LIHAT PEMBAHASAN";
                btnLihat.classList.add('w-full');
                btnLihat.onclick = q4_aksiReviewJawaban;
            }
            if(btnDash) btnDash.classList.remove('hidden');
        }
        ...
        if(btnUlangi) {
            btnUlangi.onclick = function() {
                ...
                document.querySelectorAll('[onclick^="q4_toggleRagu"], [onclick="q4_aksiCekSelesai()"]').forEach(btn => {
                ...
                if(typeof q4_initDragAndDrop === "function") setTimeout(q4_initDragAndDrop, 500);
            };
        }
    ...
    function q4_aksiMulaiUjian() {
        window.q4_isExamMode = true;
