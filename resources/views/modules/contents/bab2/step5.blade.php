<section id="step-5" class="step-slide">
    ...
    <style>
        ...
        .input-review-wrong { background-color: #fee2e2 !important; border-color: #ef4444 !important; color: #991b1b !important; }
        .input-review-correct { background-color: #dcfce7 !important; border-color: #22c55e !important; color: #14532d !important; }
        .review-feedback-box { margin-top: 20px; padding: 15px; border-radius: 8px; background-color: #f8fafc; border: 1px solid #e2e8f0; }
    ...
    <div id="quiz-wrapper">
        ...
        <div class="h-16 bg-white border-b border-slate-200 flex justify-between items-center px-6 shrink-0 z-30">
            <div>
                <h2 class="font-bold text-slate-800 text-lg">Quiz Materi 2</h2>
                <p class="text-xs text-slate-500">Implementasi & Representasi Graf di Python</p>
            </div>
        ...
        <div class="flex flex-1 overflow-hidden relative">
            ...
            <div id="area-soal" class="custom-scroll">
                            ...
                            <div class="pl-1">
                                ...
                                @if (in_array($soal->type, ['MULTIPLE_CHOICE', 'BOOLEAN']))
                                    @if (isset($soal->options) && is_iterable($soal->options))
                                        @foreach ($soal->options as $key => $val)
                                            @php
                                                // Mencegah munculnya key aneh dengan mengubah ke format A,B,C,D khusus untuk tombolnya
                                                $displayKey = in_array(strtoupper(trim($key)), ['A','B','C','D','E','F']) ? strtoupper(trim($key)) : chr(65 + $loop->index);
                                            @endphp
                                            <button type="button" class="choice-btn group" data-key="{{ $key }}"
                                                onclick="q2_aksiPilihJawaban('{{ $qid }}', '{{ $key }}', this)">
                                        ...
                                        @foreach ($soal->options as $key => $val)
                                            <label class="choice-btn flex items-center gap-4 cursor-pointer">
                                                <input type="checkbox" name="{{ $qid }}[]" value="{{ $key }}"
                                                    ...
                                                    onchange="q2_aksiCekStatus({{ $nomor }})">
                                                <span class="text-sm font-medium">{{ $val }}</span>
                                            </label>
                                        @endforeach
                                    @endif
                                ...
                                @elseif($soal->type == 'NUMBER' || $soal->type == 'FILL_BLANK')
                                    <input type="text" id="{{ $qid }}-input"
                                        ...
                                        placeholder="Ketik jawaban..." oninput="q2_aksiCekStatus({{ $nomor }})" autocomplete="off" spellcheck="false">
                                        ...
                                        @for ($i = 0; $i < $jml; $i++)
                                            <div class="flex gap-2 mb-2 items-center">
                                                ...
                                                <input type="text"
                                                    ...
                                                    oninput="q2_aksiCekStatus({{ $nomor }})" placeholder="..." spellcheck="false">
                                            </div>
                                        @endfor
                                    </div>
                ...
                <div id="navigasi-bawah" class="fixed bottom-0 left-0 right-0 md:right-[320px] h-20 bg-white border-t border-slate-200 px-8 flex justify-between items-center z-40">
                    <button id="btn-prev" onclick="q2_aksiGantiSoal(-1)" class="px-5 py-2.5 border border-slate-300 rounded-lg text-slate-600 font-bold text-sm hover:bg-slate-50 disabled:opacity-50">SEBELUMNYA</button>
                    <button id="btn-next" onclick="q2_aksiGantiSoal(1)" class="px-6 py-2.5 bg-slate-900 text-white rounded-lg font-bold text-sm hover:bg-slate-800 shadow-lg">SELANJUTNYA</button>
                </div>
            </div>
            ...
            <div id="sidebar-nav" class="hidden md:flex flex-col h-full bg-white border-l border-slate-200 w-[320px] shrink-0 z-30">
                ...
                <div class="p-5 flex-1 overflow-y-auto custom-scroll">
                    <div class="grid grid-cols-4 gap-3">
                        @foreach ($quiz2 as $index => $soal)
                            <button onclick="q2_aksiBukaSoal({{ $index + 1 }})" id="nav-btn-{{ $index + 1 }}" class="nav-btn h-10 rounded-lg font-bold text-sm transition flex items-center justify-center">
                                {{ $index + 1 }}
                            </button>
                        @endforeach
                    </div>
                    <div class="mt-6 space-y-3">
                        <button onclick="q2_toggleRagu(window.q2_soalAktif)" class="w-full py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 text-sm font-bold rounded-lg transition">Tandai Ragu-Ragu</button>
                        <button onclick="q2_aksiCekSelesai()" class="w-full py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-bold rounded-lg transition shadow-md">KUMPULKAN</button>
                    </div>
                    {{-- Legend --}}
        ...
        <div id="modal-mulai" class="fixed inset-0 bg-slate-900/80 z-[9999] flex justify-center p-4 backdrop-blur-sm -mt-8 max-w-8xl w-full">
            <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[95vh] flex flex-col">
                ...
                <div class="bg-slate-900 text-white p-4 md:p-5 text-center border-b-4 border-blue-500 shrink-0 rounded-t-2xl">
                    <h2 class="text-xl md:text-2xl font-bold tracking-wide uppercase">Quiz Materi 2 : Pemahaman Graf</h2>
                    <p class="text-slate-300 text-xs md:text-sm mt-1">Uji Pemahaman Sintaks Python Anda</p>
                </div>
                ...
                <div class="p-4 md:p-6 text-slate-700 space-y-4 overflow-y-auto custom-scroll flex-1">
                    ...
                    <div class="flex items-start gap-3 p-3 md:p-4 bg-blue-50 border border-blue-200 rounded-xl shadow-sm">
                        <div class="text-blue-600 text-xl mt-0.5"><i class="fa-solid fa-code"></i></div>
                        <div>
                            ...
                            <p class="text-xs md:text-sm text-blue-800 leading-relaxed text-justify">
                                Terdapat <strong>{{ count($quiz2) }} Tantangan</strong> yang harus Anda selesaikan. Fokus pada kemampuan Anda membaca dan menerjemahkan logika graf ke dalam bahasa pemrograman Python.
                            </p>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <ul class="space-y-3 text-xs md:text-sm text-justify">
                            <li class="flex items-start gap-3">
                                <span class="w-6 h-6 rounded-full bg-slate-200 flex items-center justify-center text-slate-700 font-bold shrink-0">1</span>
                                <span class="leading-relaxed"><strong>Durasi Waktu:</strong> Anda memiliki waktu <strong>10 Menit</strong>. Timer akan berjalan otomatis saat tombol mulai ditekan.</span>
                            </li>
                            <li class="flex items-start gap-3">
                                <span class="w-6 h-6 rounded-full bg-slate-200 flex items-center justify-center text-slate-700 font-bold shrink-0">2</span>
                                <span class="leading-relaxed"><strong>Format Code (Penting!):</strong> Pada soal isian teks, perhatikan dengan sangat teliti penggunaan tanda kurung siku <code>[]</code>, kurung biasa <code>()</code>, dan tanda kutip <code>" "</code>.</span>
                            </li>
                            <li class="flex items-start gap-3">
                                <span class="w-6 h-6 rounded-full bg-slate-200 flex items-center justify-center text-slate-700 font-bold shrink-0">3</span>
                                <span class="leading-relaxed"><strong>Fitur Navigasi:</strong> Gunakan panel di sebelah kanan layar untuk berpindah soal. Tombol <span class="bg-slate-200 px-1.5 py-0.5 rounded text-xs font-bold text-slate-600 border border-slate-300">Tandai Ragu-Ragu</span> berguna jika Anda ingin mengecek ulang.</span>
                            </li>
                            <li class="flex items-start gap-3">
                                <span class="w-6 h-6 rounded-full bg-red-100 flex items-center justify-center text-red-600 font-bold shrink-0">4</span>
                ...
                <div class="bg-slate-50 p-4 md:p-5 border-t border-slate-200 flex justify-center shrink-0 rounded-b-2xl">
                    <button onclick="q2_aksiMulaiUjian()" class="w-full md:w-auto px-8 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg transition-transform active:scale-95 text-base flex justify-center items-center gap-3">
        ...
        <div id="modal-konfirmasi" class="fixed inset-0 bg-slate-900/50 z-[9999] flex items-center justify-center p-4 hidden">
            <div class="bg-white rounded-xl p-6 max-w-sm w-full shadow-2xl transform scale-100 transition-transform">
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <button onclick="q2_aksiFinalisasi()" class="px-4 py-2 rounded bg-red-600 text-white hover:bg-red-700 font-bold text-sm shadow">YA, KUMPULKAN</button>
                </div>
            </div>
        </div>
        ...
        <div id="modal-hasil" class="fixed inset-0 bg-white z-[9999] flex items-center justify-center p-4 hidden">
            <div class="text-center w-full max-w-md mx-auto">
                ...
                <div class="flex gap-3 justify-center w-full">
                    <button onclick="q2_aksiUlangi()" class="btn-ulangi-quiz px-6 py-3 border border-slate-200 rounded-lg font-bold text-slate-600 hover:bg-slate-50 transition-colors w-full">
                        ULANGI
                    </button>
                    <button onclick="q2_aksiReviewJawaban()" class="btn-lihat-quiz px-6 py-3 bg-slate-800 text-white rounded-lg font-bold hover:bg-slate-900 shadow-lg transition-colors w-full">
                        LIHAT PEMBAHASAN
                    </button>
                    <a href="{{ route('dashboard') }}" class="btn-dashboard flex items-center justify-center px-5 py-1 border border-slate-200 rounded-lg font-bold text-slate-600 hover:bg-slate-50 transition-colors w-full">
                        DASHBOARD
                    </a>
                </div>
            </div>
...
<script>
    ...
    @php
        ...
        if (isset($progress) && isset($progress['quiz2'])) {
            $dataQuiz = $progress['quiz2'];
    ...
    // --- VARIABEL GLOBAL DENGAN NAMA FUNGSI q2_ ---
    window.q2_hasHistory = {{ $isDone ? 'true' : 'false' }};
    window.q2_historyScore = {{ $displayScore }};
    window.q2_dataSoal = @json($quiz2);

    // VARIABEL STATE
    window.q2_totalSoal = window.q2_dataSoal.length;
    window.q2_isExamMode = false;
    window.q2_soalAktif = 1;
    window.q2_sisaDetik = 600;
    window.q2_timerUjian = null;
    window.q2_dbRagu = {};

    // PARSING JAWABAN DENGAN SAFEKEEPING
    window.q2_dbJawabanUser = {};
    ...
    try {
        ...
        if (rawData) {
            ...
            window.q2_dbJawabanUser = tempDb;
        }
    } catch (e) {
    ...
    document.addEventListener("DOMContentLoaded", function() {
        if (window.q2_hasHistory) {
            ...
            if (elSkor) elSkor.innerText = window.q2_historyScore;
            ...
            setTimeout(q2_restoreUserAnswers, 500);
        } else {
            if (window.q2_totalSoal > 0) {
                q2_aksiBukaSoal(1);
                if(typeof q2_initDragAndDrop === "function") q2_initDragAndDrop();
            }
        }
    });
    ...
    // ==========================================
    function q2_restoreUserAnswers() {
        if (!window.q2_dbJawabanUser || Object.keys(window.q2_dbJawabanUser).length === 0) return;

        Object.keys(window.q2_dbJawabanUser).forEach(key => {
            let answer = window.q2_dbJawabanUser[key];
        ...
        q2_updateTampilanNav();
    }

    // ==========================================
    // 4. FUNGSI REVIEW 
    // ==========================================
    function q2_aksiReviewJawaban() {
        document.getElementById('modal-hasil').classList.add('hidden');
        if (window.q2_timerUjian) clearInterval(window.q2_timerUjian);
        const timer = document.getElementById('timer-display');
        if (timer) {
            timer.innerHTML = '<span class="text-blue-600 text-sm"><i class="fa-solid fa-eye mr-1"></i> MODE REVIEW</span>';
        ...
        if (navBawah) {
            ...
            if (!document.getElementById('btn-kembali-skor')) {
                ...
                btn.className = "px-6 py-2.5 bg-slate-900 text-white rounded-lg font-bold text-sm hover:bg-slate-800 shadow-md ml-3 transition-colors";
        ...
        document.querySelectorAll('[onclick^="q2_toggleRagu"], [onclick="q2_aksiCekSelesai()"]').forEach(btn => btn.style.display = "none");
        document.getElementById('sidebar-nav').classList.add('pointer-events-auto');

        q2_restoreUserAnswers();

        // GRADING & VISUALISASI
        window.q2_dataSoal.forEach((soal, index) => {
            let nomor = index + 1;
            let qid = 'soal_' + nomor;
            let container = document.getElementById('container-' + nomor);
            ...
            let userAnswer = window.q2_dbJawabanUser[qid];
            ...
            // FEEDBACK BOX (SOLUSI CHECKBOX 'B' DAN HIDE JIKA SALAH)
            if (feedbackArea) {
                ...
                if (isBenar) {
                    ...
                    if (soal.explanation) html += `<div class="mt-3 pt-3 border-t border-green-200 text-sm text-green-900 leading-relaxed text-justify"><strong>Pembahasan Singkat:</strong><br>${soal.explanation}</div>`;
                } else {
                    let userDisplay = (userAnswer !== undefined && userAnswer !== null && userAnswer !== "") ? userAnswer : "Kosong";
                    
                    // Terjemahkan key CHECKBOX jadi TEXT agar user tidak bingung
                    if (soal.type === "CHECKBOX" && Array.isArray(userDisplay)) {
                        let opts = soal.options;
                        if (typeof opts === 'string') {
                            try { opts = JSON.parse(opts); } catch(e) { opts = {}; }
                        }
                        let texts = userDisplay.map(k => opts[k] ? opts[k] : k);
                        userDisplay = '<div class="mt-2 flex flex-col gap-1.5">' + texts.map(t => `<div class="bg-red-50 border border-red-200 px-3 py-1.5 rounded-md font-semibold text-red-900 shadow-sm">${t}</div>`).join('') + '</div>';
                    } else {
                        if (Array.isArray(userDisplay)) userDisplay = userDisplay.join(', ');
                        else if (typeof userDisplay === 'object' && userDisplay !== "Kosong") userDisplay = JSON.stringify(userDisplay);
                        
                        if (userDisplay !== "Kosong") {
                            userDisplay = `<span class="bg-red-100 border border-red-200 px-2 py-0.5 rounded font-bold inline-block mt-1">${userDisplay}</span>`;
                        } else {
                            userDisplay = `<span class="bg-red-100 border border-red-200 px-2 py-0.5 rounded font-bold text-red-500 italic inline-block mt-1">Belum dijawab</span>`;
                        }
                    }
                    ...
                    html += `<div class="text-sm text-red-800 mt-2 font-medium">Jawaban Anda: ${userDisplay}</div>`;
                    
                    // JIKA SALAH, HANYA TAMPILKAN PESAN GENERIK (TANPA KUNCI & PENJELASAN)
                    html += `<div class="mt-5 pt-4 border-t border-red-200 text-sm text-red-900 leading-relaxed text-justify">Jawaban Anda masih belum tepat atau kosong. Kunci jawaban dan pembahasan sengaja disembunyikan agar Anda bisa mencobanya lagi!</div>`;
                }
    ...
    // 5. HELPER & HANDLERS
    // ==========================================
    function q2_updateTampilanNav() {
        for (let i = 1; i <= window.q2_totalSoal; i++) {
            ...
            let terisi = q2_cekApakahTerisi(i);
            let aktif = (i === window.q2_soalAktif);
            let ragu = window.q2_dbRagu['soal_' + i];
    ...
    function q2_cekApakahTerisi(nomor) {
        let qid = 'soal_' + nomor;
        let data = window.q2_dbJawabanUser[qid];
    ...
    function q2_aksiBukaSoal(nomor) {
        ...
        if (next) {
            if (nomor === window.q2_totalSoal) next.classList.add('hidden');
    ...
    function q2_aksiGantiSoal(arah) {
        let next = window.q2_soalAktif + arah;
        if (next >= 1 && next <= window.q2_totalSoal) q2_aksiBukaSoal(next);
    }

    function q2_toggleRagu(nomor) {
        window.q2_dbRagu['soal_' + nomor] = !window.q2_dbRagu['soal_' + nomor];
        q2_updateTampilanNav();
    }

    function q2_aksiPilihJawaban(qid, nilai, btn) {
        if (!window.q2_isExamMode) return;
        window.q2_dbJawabanUser[qid] = nilai;
    ...
    function q2_aksiCekStatus(nomor) {
        let qid = 'soal_' + nomor;
        let container = document.getElementById('container-' + nomor);
        if(!container) return;
        ...
        if (checkboxes.length > 0) {
            ...
            if (vals.length > 0) window.q2_dbJawabanUser[qid] = vals;
            else delete window.q2_dbJawabanUser[qid];
        } else {
            ...
            if (texts.length > 0) {
                ...
                if (adaIsi) window.q2_dbJawabanUser[qid] = (texts.length === 1) ? vals[0] : vals;
                else delete window.q2_dbJawabanUser[qid];
            }
        }
        q2_updateTampilanNav();
        ...
        zones.forEach(zone => {
            ...
            zone.addEventListener('drop', (e) => {
                ...
                if (containerSoal) {
                    ...
                    if (adaIsi) window.q2_dbJawabanUser[qid] = answerObj;
                    else delete window.q2_dbJawabanUser[qid];
                    q2_updateTampilanNav();
                }
            });
    ...
    // --- SUBMIT ---
    function q2_aksiCekSelesai() {
        let kosong = [];
        for (let i = 1; i <= window.q2_totalSoal; i++) {
            if (!q2_cekApakahTerisi(i)) kosong.push(i);
        }
    ...
    function q2_aksiFinalisasi() {
        if (!window.q2_isExamMode) return;
        clearInterval(window.q2_timerUjian);
        window.q2_isExamMode = false;
        ...
        fetch("{{ route('submit.quiz') }}", {
                ...
                body: JSON.stringify({
                    ...
                    section_code: 'quiz2',
                    score: 0, 
                    answers: window.q2_dbJawabanUser 
                })
            })
            .then(res => res.json())
            .then(data => {
                ...
                window.q2_hasHistory = true;
                window.q2_historyScore = data.score;
            })
            .catch(err => {
                console.error(err);
    ...
    function q2_aksiUlangi() {
        ...
        function restoreModalUI() {
            ...
            if(btnUlangi) {
                ...
                btnUlangi.onclick = q2_aksiUlangi;
            }
            if(btnLihat) {
                btnLihat.innerText = "LIHAT PEMBAHASAN";
                btnLihat.classList.add('w-full');
                btnLihat.onclick = q2_aksiReviewJawaban;
            }
            if(btnDash) btnDash.classList.remove('hidden');
        }
        ...
        if(btnUlangi) {
            btnUlangi.onclick = function() {
                ...
                document.querySelectorAll('[onclick^="q2_toggleRagu"], [onclick="q2_aksiCekSelesai()"]').forEach(btn => {
                ...
                if(typeof q2_initDragAndDrop === "function") setTimeout(q2_initDragAndDrop, 500);
            };
        }
    ...
    function q2_aksiMulaiUjian() {
        window.q2_isExamMode = true;
        ...
        if (window.q2_timerUjian) clearInterval(window.q2_timerUjian);
        window.q2_timerUjian = setInterval(() => {
            window.q2_sisaDetik--;
            let m = Math.floor(window.q2_sisaDetik / 60).toString().padStart(2, '0');
            let s = (window.q2_sisaDetik % 60).toString().padStart(2, '0');
            if (timerDisp) {
                timerDisp.innerText = m + ":" + s;
                if (window.q2_sisaDetik <= 60) timerDisp.classList.add('text-red-600', 'animate-pulse');
            }
            if (window.q2_sisaDetik <= 0) q2_aksiFinalisasi();
        }, 1000);
    }
</script>